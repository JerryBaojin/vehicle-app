...
<script>
	...
	export default {
		...
		activated() {
			initScroll(this.$refs.wrapper, this.$store.state.officialRecord);
		},
		deactivated() {
			if(this.$store.state.officialRecord != this.scroll) {
				this.$store.dispatch('upDataOfficial', this.scroll);
			}			
		}
	}
</script>

					...
					<ul class="cont-wrap">
						<li v-for="(item,index) in dataArray" @click="goDetail">
							<div>
								...
								<p class="left-message">保养类型：日常保养</p>
							</div>
...
//				console.log(this.scroll)
			...
			//去详情页面
			goDetail() {
				this.$router.push('/upkeepDetail?id=22');
			},
			//去新增页面
			toAdd() {
				this.$router.push('/upkeepAdd');				
			}
		},
		created() {
		...
		activated() {
			initScroll(this.$refs.wrapper, this.$store.state.upkeep);
		},
		deactivated() {
			if(this.$store.state.upkeep != this.scroll) {
				this.$store.dispatch('upDataUpkeep', this.scroll);
			}			
		}
	}
</script>

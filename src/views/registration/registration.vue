...
<script>
	...
	export default {
		...
		methods: {
			...
			//去详情页面
			goDetail() {
				this.$router.push('/registrationDetail?id=22');
			}
		},
		created() {
		...
		activated() {
			initScroll(this.$refs.wrapper, this.$store.state.registration);
		},
		deactivated() {
			if(this.$store.state.registration != this.scroll) {
				this.$store.dispatch('upDataRegistration', this.scroll);
			}			
		}
	}
</script>

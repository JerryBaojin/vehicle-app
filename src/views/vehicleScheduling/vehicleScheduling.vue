...
//				console.log(this.scroll)
			...
			//去详情页面
			goDetail() {
				this.$router.push('/schedulingDetail?id=22');
			}
		},
		created() {
		...
		activated() {
			initScroll(this.$refs.wrapper, this.$store.state.vehicleScheduling);
		},
		deactivated() {
			if(this.$store.state.vehicleScheduling != this.scroll) {
				this.$store.dispatch('upDataScheduling', this.scroll);
			}			
		}
	}
</script>

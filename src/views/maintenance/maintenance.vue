					...
					<ul class="cont-wrap">
						<li v-for="(item,index) in dataArray" @click="goDetail">
							<div>
								...
								<p class="left-message">维修类型：钣金维修</p>
							</div>
...
//				console.log(this.scroll)
			...
			//去详情页面
			goDetail() {
				this.$router.push('/maintenanceDetail?id=22');
			},
			//去新增页面
			toAdd() {
		...
		activated() {
			initScroll(this.$refs.wrapper, this.$store.state.maintenance);
		},
		deactivated() {
			if(this.$store.state.maintenance != this.scroll) {
				this.$store.dispatch('upDataMaintenance', this.scroll);
			}			
		}
	}
</script>
